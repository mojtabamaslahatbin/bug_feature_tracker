...
import {ItemInterface, PersonInterface, ViewMode} from "../../Types/types";
...
const AssignedToMe: React.FC = () => {
    ...
    useEffect(() => {
        if (state.items) {
            ...
            const filteredItems = cloneItems.filter(item =>
                item.assignedTo?.some((x: PersonInterface) => x.user_id === state.appUser?.user_id)
            );
            setUserItems(filteredItems);
        }
    }, [state.items, state.appUser?.user_id]);
